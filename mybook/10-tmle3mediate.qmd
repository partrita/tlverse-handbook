...
인과 추론에 대한 현대적 접근 방식은 전통적인 경로 분석의 방법론을 크게 발전시켜 파라메트릭 모델링 접근 방식의 사용으로 인한 중대한 제한 사항을 극복할 수 있게 해주었습니다[@vanderweele2015explanation]. 서로 다른 프레임워크를 사용하여 @robins1992identifiability와 @pearl2001direct는 평균 치료 효과를 *자연적* 직접 및 간접 효과로 비파라메트릭하게 분해하는 방법을 동일하게 제공했습니다. @vanderweele2015explanation은 고전적 인과 매개 분석에 대한 포괄적인 개요를 제공합니다. 우리는 대안적인 관점을 제공하며, 대신 최근에야 나타난 이러한 수치들의 효율적 추정기 구축[@tchetgen2012semiparametric; @zheng2012targeted]과 확률적 중재에 기반한 보다 유연한 직접 및 간접 정의[@diaz2020causal]에 초점을 맞춥니다.
...
우리는 노출에 의해 영향을 받는 매개체-결과 관계의 잠재적 교란 요인(즉, $A$에 의해 영향을 받고 $Z$와 $Y$ 모두에 영향을 미치는 변수)을 명시적으로 제외했습니다. 이러한 변수가 존재하는 경우의 매개 분석은 매우 어렵습니다 [@avin2005identifiability]. 따라서 인과적 직접 및 간접 효과의 정의를 개발하려는 대부분의 노력은 이러한 교란 요인이 없음을 명시적으로 가정합니다. 추가적인 가정이 없다면, 이러한 교란이 있는 경우 일반적인 매개 매개변수(자연적 직접 및 간접 효과)를 식별할 수 없습니다. 하지만 @tchetgen2014identification은 연구 중인 시스템에 대한 과학적 지식에 의해 정당화될 때 유용할 수 있는 단조성 가정을 논의합니다. 관심 있는 독자는 최근 급격히 성장하고 있는 인과 매개 분석 문헌의 성과인 *중재적*(interventional) 직접 및 간접 효과 [@didelez2006direct; @vanderweele2014effect; @lok2016defining; @vansteelandt2017interventional; @rudolph2017robust; @nguyen2019clarifying]를 참고할 수 있습니다. 이들의 식별은 이러한 복잡한 형태의 치료 후 교란에 강건합니다. 이 흐름의 문헌 내에서 @diaz2020nonparametric 및 @benkeser2020nonparametric은 비파라메트릭 효과 분해 및 효율성 이론에 대한 고찰을 제공하며, @hejazi2021nonparametric은 확률적 중재를 활용하는 새로운 부류의 효과를 공식화합니다.
...
\end{align*}
특히, 자연적 간접 효과(natural indirect effect, NIE)는 매개체 $Z$를 통한 치료 $A \in \{0, 1\}$가 결과 $Y$에 미치는 효과를 측정하며, 자연적 직접 효과(natural direct effect, NDE)는 *다른 모든 경로를 통한* 치료가 결과에 미치는 효과를 측정합니다. 자연적 직접 및 간접 효과의 식별에는 다음과 같은 검증 불가능한 인과적 가정이 필요합니다. 우리가 고려하는 SCM이 독립적이고 동일하게 분포된(iid) 단위만을 생성하도록 제한되어 있고, 반사실이 유도된 수치이기 때문에(잠재적 결과 프레임워크에서의 원시 수치와 반대됨) 일관성과 간섭 부재(전통적으로 SUTVA [@rubin1978bayesian; @rubin1980randomization])라는 표준 가정이 성립한다는 점에 유의하십시오. @pearl2010brief는 후자의 지점에 대해 영감을 주는 논의를 제공합니다.

::: {.definition name="교환 가능성"}
$Y(a, z) \indep (A, Z) \mid W$이며, 이는 $\E\{Y(a, z) \mid A=a, W=w, Z=z\} \equiv \E\{Y(a, z) \mid W=w\}$임을 암시합니다. 이는 매개체가 존재할 때 보이지 않는 교란 요인이 없다는 표준 가정의 특별하고 더 제한적인 사례입니다. 대응하는 무작위 배정 가정은 단순히 치료 $A$와 매개체 $Z$ 모두에 대한 합동 중재에 적용된 표준 무작위 배정 가정입니다.
:::

::: {.definition name="치료 양의 조건"}
모든 $a \in \mathcal{A}$ 및 $w \in \mathcal{W}$에 대해, 치료의 조건부 확률 $g(a \mid w)$는 작은 계수 $\xi > 0$에 의해 단위 구간의 한계로부터 떨어져 있습니다. 더 정확하게는 $\xi < g(a \mid w) < 1 - \xi$입니다. 이는 [이전](#tmle3)에 논의된 정적 중재에 필요한 표준 양의 조건 가정을 따릅니다.
:::

::: {.definition name="매개체 양의 조건"}
모든 $z \in \mathcal{Z}$, $a \in \mathcal{A}$ 및 $w \in \mathcal{W}$에 대해, 매개체의 조건부 밀도는 작은 계수 $\epsilon > 0$에 의해 0으로부터 떨어져 있어야 합니다. 구체적으로 $\epsilon < q_{0,Z}(z \mid a, w)$입니다. 본질적으로 이는 모든 합동 지지 집합(joint support) $\mathcal{Z} \times \mathcal{A} \times \mathcal{W}$ 내의 $\{z, a, w\}$에 대해 매개체의 조건부 밀도가 0보다 커야 함을 의미합니다. 즉, 치료 $A$와 기본 공변량 $W$에 의해 정의된 모든 계층에서 주어진 매개체 값을 관찰할 수 있어야 합니다. 이 가정의 덜 제한적인 형태도 가능합니다. 구체적으로, 서로 다른 치료 대조 하에서 매개체 밀도의 두 실현값에 대해 두 치료 대조 하의 매개체 밀도 비율이 유계여야 한다는 것입니다.
:::

::: {.definition name="교차 세계 반사실적 독립성"}
...
처음 세 가지 가정은 더 단순한 설정의 대응물들을 통해 익숙할 수 있지만, 교차 세계 독립성 요건은 자연적 직접 및 간접 효과의 식별에 고유한 것입니다. 이 가정은 이러한 경로 특정 효과의 식별에 있어 까다로운 복잡성을 해결하는데, @avin2005identifiability는 이를 "위증하는 증인(recanting witness)"이라 명명하고 이 가정과 동등한 그래픽적 해결책을 도입했습니다. 서로 다른 중재에 의해 지표화된 반사실들의 이러한 독립성은 사실 이러한 효과 정의의 과학적 관련성에 대한 심각한 제한입니다. 왜냐하면 이는 무작위 시험에서도 NDE와 NIE를 식별할 수 없게 만들고 [@robins2010alternative], 대응하는 과학적 주장이 실험을 통해 허위임을 입증할 수 없음을 의미하며 [@popper1934logic; @dawid2000causal], 결과적으로 과학적 방법의 기초적인 기둥과 직접적으로 모순되기 때문입니다.

이 마지막 가정을 약화시키려는 많은 시도가 있었지만 [@petersen2006estimation; @imai2010identification; @vansteelandt2012imputation; @vansteelandt2012natural], 이러한 결과들은 엄격한 모델링 가정을 부과하거나, 자연적 효과에 대한 대안적인 해석을 제안하거나, 더 쉽게 충족될 수 있는 조건을 개발함으로써 제한된 수준의 추가적인 유연성만을 제공합니다. 예를 들어, @petersen2006estimation은 이 가정이 (별개의 반사실이 아닌) 조건부 평균에 대해서만 요구되도록 약화시키고, 자연적 직접 효과를 또 다른 유형의 직접 효과인 통제된 직접 효과(controlled direct effect)의 가중 평균으로 보는 관점을 채택합니다. 의욕 있는 독자는 이러한 세부 사항을 독립적으로 더 조사해 볼 수 있습니다. 다음으로 우리는 현대의 인과 매개 분석 응용 분야에서 여전히 널리 사용되는 NDE와 NIE의 추정을 검토합니다.
...
\begin{equation*}
  ...
  - g(1\mid w)},
\end{equation*}
여기서 스칼라 $0 < \delta < \infty$는 *치료를 받을 확률의 변화*를 지정합니다. 인과 매개 분석의 맥락에서 @diaz2020causal이 설명했듯이, PIDE와 PIIE에 필요한 식별 가정은 NDE와 NIE에 필요한 가정보다 훨씬 관대합니다. 이러한 식별 가정은 다음과 같습니다. 중요한 점은, 교차 세계 반사실적 독립성 가정이 전혀 필요하지 않다는 것입니다.

::: {.definition name="치료와 매개체의 조건부 교환 가능성"}
모든 $(a, z) \in \mathcal{A} \times \mathcal{Z}W$에 대해 $\E\{Y(a, z) \mid Z, A, W\} = \E\{Y(a, z) \mid Z, W\}$라고 가정합니다. 이 가정은 @vansteelandt2012natural이 치료군 사이의 매개 효과 식별을 위해 원래 제안한 $Y(a, z) \indep (A,Z) \mid W$ 가정보다 강력하며 이에 의해 함의됩니다. @diaz2020causal은 이 가정을 도입하면서 다음과 같이 기술했습니다: "이 가정은 노출과 매개체가 무작위 배정된 무작위 실험에서 노출 전 공변량 $W$에 대해 충족될 것입니다. 따라서 인구 중재에 대한 직접 효과는 평균 치료 효과에 대한 자연적 직접 효과[@robins2010alternative]와 달리, $A$와 $Z$에 대한 중재를 통한 무작위 실험의 치료 요법 간의 대조에 대응합니다."
:::
<!-- TODO: explain this-->

::: {.definition name="치료와 매개체의 공통 지지 집합"}
모든 $w \in \mathcal{W}$에 대해 $\text{supp}\{g_{\delta}(\cdot \mid w)\} \subseteq \text{supp}\{g(\cdot \mid w)\}$라고 가정합니다. 이 가정은 표준적이며, 중재 후의 $A$ 값이 데이터에서 지지되어야 함만을 요구합니다. 이는 자연적 직접 및 간접 효과에 필요한 치료 및 매개체 양의 조건보다 훨씬 약하며, 이는 정적이 아닌 확률적 중재를 사용한 결과입니다.
:::
<!-- TODO: explain this-->
...
@diaz2020causal이 설명했듯이, PIDE와 PIIE에 모두 나타나는 분해 항 $\E\{Y(A_{\delta}, Z)\}$(매개 기전은 고정된 상태로 유지하면서 치료 기전을 변경하는 것에 대응함)를 식별하는 통계적 기능어는 다음과 같습니다:
\begin{equation*}

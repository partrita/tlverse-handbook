...
```
<!--
우리는 이 단순화된 데이터를 사용하여 평균 치료 효과(ATE)를 추정하는 데 관심이 있습니다.
$\Psi(P_0)=\mathbb{E}_0(\mathbb{E}_0[Y \mid A=1,W]-\mathbb{E}_0[Y \mid A=0,W])$

순수하게 통계적인(비인과적) 파라미터는 W의 계층에 걸친 평균 차이의 평균으로 해석될 수 있으며, 각 가능한 w에 대해 조건부 치료 할당 확률이 양수라는 양의 조건 가정만 필요합니다: $\mathbb{P}_0(A=1 \mid W=w) > 0$ 및 $\mathbb{P}_0(A=0 \mid W=w) > 0$.

이 파라미터를 인과적으로, 특히 인과적 위험 차이 $E_0Y_1-E_0Y_0$로 해석하려면, W의 계층 내에서 A가 반사실적 결과 $(Y_0,Y_1)$와 독립적이라는 무작위화 가정도 세워야 합니다. 이 가정은 원래 SCM $\mathcal{M}^F$에 포함되었을 수 있지만, 만약 측정되지 않은 혼란 변수가 있다는 것을 안다면 모델 $\mathcal{M}^{F\*}$은 이 "틀린 것으로 알려진" 무작위화 가정을 강제함으로써 더 제한적일 것입니다. 그럼에도 불구하고 이 가정은 통계 모델 $\mathcal{M}$을 바꾸지 않으며 결과적으로 추정 문제에도 영향을 미치지 않습니다. 따라서 TMLE의 바람직한 속성을 확립하는 정리들은 이 테스트 불가능한 무작위화 가정이 위반될 때에도 여전히 유효합니다.

우리는 점근적으로 일관되고 점근적으로 정규 분포를 따르며 점근적으로 효율적일 뿐만 아니라 견고한 유한 표본 성능을 갖도록 맞춤화된 효율적인 대입 추정기인 타겟 최소 손실 기반 추정기(TMLE)를 구현하는 과정을 진행합니다.
-->
